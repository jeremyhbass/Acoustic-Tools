...
def polarShow(df, polarTitle, sWS, eWS, data_type):
    ...
    r = df['T01-WindSpd']
    theta = np.radians(df['T01-WindDirn'])
...
turbs = ['T01','T03','T05','T06','T08','T09']
for index, turb in enumerate(turbs):
    spd  = turb + '-WindSpd'
    dirn = turb + '-WindDirn'
    # print spd, dirn
    data['DirDiff'] = data[dirn] - data['T07-WindDirn']
    # If < -180 deg, add 360 deg
    data['DirDiff'] = [wspd + 360 if wspd < -180 and wspd is not np.NaN else wspd for wspd in data['DirDiff']]
    # if > 180 deg, subtract 360 deg
    data['DirDiff'] = [wspd - 360 if wspd > +180 and wspd is not np.NaN else wspd for wspd in data['DirDiff']]
    offset = data['DirDiff'].mean()
    print 'Index: {0:1n}  Turbine: {1:s}  Offset:{2:6.1f} deg'.format(index, turb, offset)
    # Add checks
    plt.figure()
    plt.axis([0, 360, -180, +180])        
    # plt.plot(data['T07-WindSpd'], data[spd],'r.', alpha=0.3, label =turb)
    # plt.plot(data['T07-WindDirn'], data[dirn],'r.', alpha=0.3, label =turb)
    plt.hlines(offset, 0, 360, colors='k', label = 'Offset {0:6.1f} deg'.format(offset), zorder=1)
    plt.plot(data['T07-WindDirn'], data['DirDiff'],'r.', alpha=0.3, label =turb, zorder=2)
    plt.xlabel('Hub Height Wind Direction / deg')
    plt.ylabel('Hub Height Wind Direction / deg')
    plt.grid('on',which=u'major', axis=u'both')
    plt.legend(loc='best')
    plt.title('Comparison of Turbine Wind Direction with T07 Direction: 22/02/21-06/06/21')
    # plt.tight_layout()
    surveyFigName = inputFile[:-5] + "_" + turb + "_Dir_Scatter.png"
...
# polar_plot(data_1,   "Wind Polar Plot: " + startDay_1 + "-" + stopDay_1 + " - 42 dB(A) Lden Limit - D/E/N", 3, 12, G_RANGE, inputFile[:-5], 1011, 0)
...
# polar_plot(data_2,   "Wind Polar Plot: " + startDay_2 + "-" + stopDay_2 + " - 42 dB(A) Lden Limit + New AM Mode 2 - D/E/N", 3, 12, G_RANGE, inputFile[:-5], 1012, 0)
...
# polar_plot(data_1,   "AM Polar Plot: " + startDay_1 + "-" + stopDay_1 + " - 42 dB(A) Lden Limit - D/E/N", 3, 12, G_RANGE, inputFile[:-5],  1011, 1)
...
# polar_plot(data_2,   "AM Polar Plot: " + startDay_2 + "-" + stopDay_2 + " - 42 dB(A) Lden Limit + New AM Mode 2 - D/E/N", 3, 12, G_RANGE, inputFile[:-5],  1012, 1)
